...
import { storage, db } from "@/app/api/memories/firebase";
...
export default function MemoriesPage() {
  ...
  const seedInitialData = async () => {
    ...
    try {
      // For each section
      for (const [section, memories] of Object.entries(initialData)) {
        // For each memory in the section
        for (const memory of memories) {
          // Check if memory already exists in Firestore
          const q = query(collection(db, `memories_${section}`));
          const querySnapshot = await getDocs(q);
          const exists = querySnapshot.docs.some(doc => doc.data().id === memory.id);

          if (!exists) {
            // Add to Firestore with timestamp
            await addDoc(collection(db, `memories_${section}`), {
              ...memory,
  ...
  const fetchMemories = async () => {
    try {
      const fetchSection = async (sectionType: string) => {
        const q = query(
          collection(db, `memories_${sectionType}`),
      ...
      const [birthData, weddingData, vacationData] = await Promise.all([
        fetchSection('birth'),
        fetchSection('wedding'),
        fetchSection('vacation'),
      ]);

      setBirthMemories(birthData);
      setWeddingMemories(weddingData);
      setVacationMemories(vacationData);
    } catch (error) {
      console.error('Error fetching memories:', error);
    }
  };

  // Initialize data and fetch on component mount
  useEffect(() => {
